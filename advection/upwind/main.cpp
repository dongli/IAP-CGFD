...
int main(int argc, const char *argv[])
{
    ...
    double dt, dx;
    string outputPattern = "upwind.%3s.nc";
    ...
    dt = configManager.getValue("upwind", "dt", 1.0);
    dx = configManager.getValue("upwind", "dx", 0.01);
    outputPattern = configManager.getValue("upwind", "output_pattern", outputPattern);
    ...
    while (!timeManager.isFinished()) {
        ...
        for (int i = mesh.is(HALF); i <= mesh.ie(HALF); ++i) {
            fu(i) = dt/dx*0.5*(     u(halfIdx, i) *(f(oldIdx, i+1)+f(oldIdx, i))-
                               fabs(u(halfIdx, i))*(f(oldIdx, i+1)-f(oldIdx, i)));
        }
        fu.applyBndCond();
        for (int i = mesh.is(FULL); i <= mesh.ie(FULL); ++i) {
